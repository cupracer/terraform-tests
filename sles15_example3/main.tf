...
resource "libvirt_domain" "system" {
  ...
  provisioner "file" {
    source      = "${path.module}/pillar/top.sls"
    destination = "/srv/pillar/top.sls"
  }

  provisioner "file" {
    destination = "/srv/pillar/timezone.sls"
    content     = templatefile("${path.module}/pillar/timezone.tftpl", {
  ...
  provisioner "file" {
    destination = "/srv/pillar/registration.sls"
    content     = templatefile("${path.module}/pillar/registration.tftpl", {
      register_email = var.register_email,
      register_key = var.register_key,
    })
  }
